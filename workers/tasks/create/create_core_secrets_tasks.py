...
GIT_BRANCH_PREFIX = "auto/create-core-secrets-"

SKIP_VERIFY_TASKS = True

@shared_task(bind=True, name="workers.tasks.create.create_core_secrets", queue="core_secrets")
@track_step
def create_core_secrets(self, prior:dict, **kwargs):
    # Observe what Celery actually delivered
    logger.debug("[create_core_secrets] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[create_core_secrets] created branch %s", branch_name)
            src = f"{INFR_BASE_PATH_DEMO}/alpha/xib-secrets"
            dst = f"{INFR_BASE_PATH_DEMO}/{project_id}/xib-secrets"
            ...
            if gh.remote_branch_exists(branch_name):
                result = {
                    ...
                    "branch_exists": True,
                    "note": f"[create:core_secrets] Branch {branch_name} already exists; skipping push."
                }
                return result
            gh.commit_and_push(f"provisioner core_secrets {project_id}")
            return {
                "project_id": project_id,
                "workflow_id": prior["workflow_id"],
                "branch_name": branch_name,
                "branch_exists": False,
                "note": f"[create:core_secrets] started with {branch_name}"
            }
    except CeleryRetry:
        raise
...
@shared_task(bind=True, name="workers.tasks.create.create_core_secrets_mr", queue="core_secrets")
@track_step
def create_core_secrets_mr(self, prior:dict, **kwargs):
    ...
    try:
        ...
        mr = get_or_create_mr(
            ...
            squash=False,
            labels=["create", "core_secrets"],
            description=f"Automated MR for `{project_id}`",
        )
        logger.info("Core_Secrets MR ready: %s", getattr(mr, "web_url", "<no-url>"))
...
@shared_task(bind=True, name="workers.tasks.create.deploy_core_secrets", max_retries=5,
             retry_delay=timedelta(minutes=2))
@track_step
def deploy_core_secrets(self, prior:dict, *, mr_iid: int | None = None):
    ...
    try:
        ...
        source_branch = f"{GIT_BRANCH_PREFIX}{project_id}"
        atlantis_dir       = f"{INFR_BASE_PATH_DEMO}/{project_id}/xib-secrets"
...
@shared_task(bind=True, name="workers.tasks.create.verify_core_secrets",
             retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
def verify_core_secrets(self, prior: dict):
    if SKIP_VERIFY_TASKS:
        logger.warning("[verify_core_secrets] Skipping Verification of core_secrets")
        return {**prior}
    ...
    secret_specs = [
        ...
        SecretSpec(namespace=namespace, names=["xibus-shared-secrets", "xibus-shared-keystore", "xibus-shared-truststore", "xibus-inbound-sftp-secret", "xibus-deliver-sftp-secret"]),
    ]
...
@shared_task(bind=True, name="workers.tasks.create.merge_core_secrets_mr",
             retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
def merge_core_secrets_mr(self, prior:dict, **kwargs):
    logger.debug("[merge_core_secrets] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
    ...
    try:
        merge_results = merge_branch(
            ...
            log_prefix="[merge_core_secrets_mr]",
        )
