...
GIT_BRANCH_PREFIX = "auto/create-core-ns-"

SKIP_VERIFY_TASKS = True

@shared_task(bind=True, name="workers.tasks.create.create_core_ns", queue="core_ns")
@track_step
def create_core_ns(self, prior:dict, **kwargs):
    # Observe what Celery actually delivered
    logger.debug("[create_core_ns] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[create_core_ns] created branch %s", branch_name)
            src = f"{INFR_BASE_PATH_DEMO}/alpha/xib-namespace"
            dst = f"{INFR_BASE_PATH_DEMO}/{project_id}/xib-namespace"
            ...
            if gh.remote_branch_exists(branch_name):
                result = {
                    ...
                    "branch_exists": True,
                    "note": f"[create:core_ns] Branch {branch_name} already exists; skipping push."
                }
                return result
            gh.commit_and_push(f"provisioner core_ns {project_id}")
            return {
                "project_id": project_id,
                "workflow_id": prior["workflow_id"],
                "branch_name": branch_name,
                "branch_exists": False,
                "note": f"[create:core_ns] started with {branch_name}"
            }
    except CeleryRetry:
        raise
...
@shared_task(bind=True, name="workers.tasks.create.create_core_ns_mr", queue="core_ns")
@track_step
...
@shared_task(bind=True, name="workers.tasks.create.deploy_core_ns", max_retries=5,
             retry_delay=timedelta(minutes=2))
@track_step
def deploy_core_ns(self, prior:dict, *, mr_iid: int | None = None):
    logger.debug("[create_core_ns] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
...
@shared_task(bind=True, name="workers.tasks.create.verify_core_ns",
...
@shared_task(bind=True, name="workers.tasks.create.merge_core_ns_mr",
             retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
