...
GIT_BRANCH_PREFIX = "auto/create-core-networking-"
...
@shared_task(bind=True, name="workers.tasks.create.create_core_networking", queue="core_networking")
@track_step
def create_core_networking(self, prior:dict, **kwargs):
    # Observe what Celery actually delivered
    logger.debug("[create_core_networking] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[create_core_networking] created branch %s", branch_name)
            src = f"{INFR_BASE_PATH_DEMO}/alpha/networking"
            dst = f"{INFR_BASE_PATH_DEMO}/{project_id}/networking"
            ...
            if gh.remote_branch_exists(branch_name):
                result = {
                    ...
                    "branch_exists": True,
                    "note": f"[create:core_networking] Branch {branch_name} already exists; skipping push."
                }
                return result
            gh.commit_and_push(f"provisioner core_networking {project_id}")
            return {
                "project_id": project_id,
                "workflow_id": workflow_id,
                "branch_name": branch_name,
                "branch_exists": False,
                "note": f"[create:core_networking] started with {branch_name}"
            }
    except CeleryRetry:
        raise
...
@shared_task(bind=True, name="workers.tasks.create.create_core_networking_mr", queue="core_networking")
@track_step
...
@shared_task(bind=True, name="workers.tasks.create.deploy_core_networking", max_retries=5,
             retry_delay=timedelta(minutes=2))
@track_step
def deploy_core_networking(self, prior:dict, *, mr_iid: int | None = None):
...
@shared_task(bind=True, name="workers.tasks.create.verify_core_networking",
...
@shared_task(bind=True, name="workers.tasks.create.merge_core_networking_mr",
             retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
