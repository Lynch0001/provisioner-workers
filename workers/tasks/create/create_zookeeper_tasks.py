...
from workers.handlers.git_handler import GitHandler, GitRepoSpec
...
GIT_BRANCH_PREFIX = "auto/create-zk-"

SKIP_VERIFY_TASKS = True

@shared_task(bind=True, name="workers.tasks.create.create_zookeeper_cluster", queue="zookeeper")
@track_step
def create_zookeeper_cluster(self, prior:dict, **kwargs):
    # Observe what Celery actually delivered
    logger.debug("[create_zookeeper_cluster] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[create_zookeeper_cluster] created branch %s", branch_name)
            src = f"{INFR_BASE_PATH_DEMO}/alpha/zookeeper"
            dst = f"{INFR_BASE_PATH_DEMO}/{project_id}/zookeeper"
            ...
            if gh.remote_branch_exists(branch_name):
                result = {
                    ...
                    "branch_exists": True,
                    "note": f"[create:zookeeper] Branch {branch_name} already exists; skipping push."
                }
                return result
            gh.commit_and_push(f"provisioner zookeeper {project_id}")
            return {
                "project_id": project_id,
                "workflow_id": prior["workflow_id"],
                "branch_name": branch_name,
                "branch_exists": False,
                "note": f"[create:zookeeper] started with {branch_name}"
            }
    except CeleryRetry:
        raise
...
@shared_task(bind=True, name="workers.tasks.create.create_zookeeper_mr", queue="zookeeper")
@track_step
...
@shared_task(bind=True, name="workers.tasks.create.deploy_zookeeper_cluster", queue="zookeeper", max_retries=5, retry_delay=timedelta(minutes=2))
@track_step
def deploy_zookeeper_cluster(self, prior:dict, *, mr_iid: int | None = None):
...
@shared_task(bind=True, name="workers.tasks.create.verify_zookeeper_cluster",
...
def verify_zookeeper_cluster(self, prior: dict):
    if SKIP_VERIFY_TASKS:
        logger.warning("[verify_zookeeper_cluster] Skipping Verification of Zookeeper cluster")
        return {**prior}
...
@shared_task(bind=True, name="workers.tasks.create.merge_zookeeper_mr",
             queue="zookeeper", retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
