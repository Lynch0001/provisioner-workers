...
from workers.handlers.verify_handler import WorkloadSpec, SecretSpec
...
GIT_BRANCH_PREFIX = "auto/create-art-"
SKIP_VERIFY_TASKS = True

@shared_task(bind=True, name="workers.tasks.create.create_artemis_cluster", queue="artemis")
@track_step
def create_artemis_cluster(self, prior:dict, **kwargs):
    # Observe what Celery actually delivered
    logger.debug("[create_artemis_cluster] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[create_artemis_cluster] created branch %s", branch_name)
            src = f"{INFR_BASE_PATH_DEMO}/alpha/artemis-operator-broker"
            dst = f"{INFR_BASE_PATH_DEMO}/{project_id}/artemis-operator-broker"

            gh.copy_tree(src, dst, dirs_exist_ok=True)
            # --- Replace tokens across ALL relevant files under Artemis ---
            # terragrunt.hcl lives in many subfolders (operator-broker, etc.)
            replaced = gh.replace_tokens_in_tree(
                dst,
                {"alpha": project_id},
                file_globs=("terragrunt.hcl", "*.hcl", "*.yaml", "*.yml"),
            )
            if replaced == 0:
                # not fatal, but helpful to see if nothing matched:
                # raise or retry depending on your policy
                raise self.retry(countdown=60, exc=RuntimeError(
                    f"No files updated under {dst}; check template structure or token"
                ))
            if gh.remote_branch_exists(branch_name):
                result = {
                    ...
                    "branch_exists": True,
                    "note": f"[create:artemis] Branch {branch_name} already exists; skipping push."
                }
                return result

            gh.commit_and_push(f"provisioner artemis {project_id}")
            return {
                "project_id": project_id,
                "workflow_id": prior["workflow_id"],
                "branch_name": branch_name,
                "branch_exists": False,
                "note": f"[create:artemis] started with {branch_name}"
            }
...
@shared_task(bind=True, name="workers.tasks.create.create_artemis_mr", queue="artemis")
@track_step
...
@shared_task(bind=True, name="workers.tasks.create.deploy_artemis_cluster", queue="artemis", max_retries=5,
             retry_delay=timedelta(minutes=2))
@track_step
def deploy_artemis_cluster(self, prior:dict, *, mr_iid: int | None = None):
    logger.debug("[create_artemis] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
...
@shared_task(bind=True, name="workers.tasks.create.verify_artemis_cluster",
...
@shared_task(bind=True, name="workers.tasks.create.merge_artemis_mr",
             queue="artemis", retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
