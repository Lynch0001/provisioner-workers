...
@track_step
def create_core_project(self, prior:dict, **kwargs):
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[create_core_project] created branch %s", branch_name)
            # project terragrunt.hcl
            src = f"{INFR_BASE_PATH_DEMO}/projects/alpha"
            dst = f"{INFR_BASE_PATH_DEMO}/projects/{project_id}"
            gh.copy_tree(src, dst, dirs_exist_ok=True)
            gh.replace_tokens_in_file(f"{dst}/terragrunt.hcl", {"alpha": project_id})
            # # project project.hcl
            src = f"{INFR_BASE_PATH_DEMO}/alpha/project.hcl"
            dst_dir = f"{INFR_BASE_PATH_DEMO}/{project_id}"
            gh.mkdir(dst_dir)
            dst_file = f"{dst_dir}/project.hcl"
            gh.copy(src, dst_file)
            gh.replace_tokens_in_file(f"{dst_dir}/project.hcl", {"alpha": project_id})
            if gh.remote_branch_exists(branch_name):
                result = {
                    "project_id": project_id,
                    "branch_name": branch_name,
                    "branch_exists": True,
                    "note": f"[create:core_project] Branch {branch_name} already exists; skipping push."
                }
                return result
            gh.commit_and_push(f"provisioner core_project {project_id}")
            return {
                "project_id": project_id,
                "workflow_id": prior["workflow_id"],
                "branch_name": branch_name,
                "branch_exists": False,
                "note": f"[create:core_project] started with {branch_name}"
            }
    except CeleryRetry:
        raise
...
@shared_task(bind=True, name="workers.tasks.create.create_core_project_mr", queue="core_project")
@track_step
...
@shared_task(bind=True, name="workers.tasks.create.core_project_wait_atlantis_plan",
...
@shared_task(bind=True, name="workers.tasks.create.deploy_core_project", max_retries=5,
             retry_delay=datetime.timedelta(minutes=2))
@track_step
def deploy_core_project(self, prior:dict, *, mr_iid: int | None = None):
    logger.debug("[create_core_project] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
...
@shared_task(bind=True, name="workers.tasks.create.core_project_wait_atlantis_apply",
...
@shared_task(bind=True, name="workers.tasks.create.verify_core_project",
...
@shared_task(bind=True, name="workers.tasks.create.merge_core_project_mr",
             retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
