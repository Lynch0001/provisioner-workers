...
GIT_BRANCH_PREFIX = "auto/destroy-core-secrets-"

SKIP_VERIFY_TASKS = True
SKIP_DESTROY_TASKS = True

@shared_task(bind=True, name="workers.tasks.destroy.delete_core_secrets", queue="core")
@track_step
def delete_core_secrets(self, prior:dict, **kwargs):
    logger.debug("[delete_secrets] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[delete_core_secrets] created branch %s", branch_name)

            repo_root = Path(gh.repo.working_tree_dir).resolve()

            rel_path = f"{INFR_BASE_PATH_DEMO}/{project_id}/xib-secrets"
            info = safe_delete_path(repo_root, rel_path)
            logger.info("[delete_core_secrets] delete info: %s", info)
            ...
            if rc != 0:
                gh.commit_and_push(f"Delete core_secrets for {project_id}")
            else:
                logger.info("[delete_core_secrets] no staged changes; skipping commit/push")
            ...
            return {
                ...
                "branch_exists": False,
                "note": f"[delete:core_secrets] started with {branch_name}"
            }
    except CeleryRetry:
        raise
...
@shared_task(bind=True, name="workers.tasks.destroy.create_core_secrets_mr", queue="core")
@track_step
def create_core_secrets_mr(self, prior:dict, **kwargs):
    ...
    try:
        ...
        mr = get_or_create_mr(
            ...
            squash=False,
            labels=["destroy", "core_secrets"],
            description=f"Automated MR for `{project_id}`",
        )
        logger.info("Secrets MR ready: %s", getattr(mr, "web_url", "<no-url>"))
...
@shared_task(bind=True, name="workers.tasks.destroy.destroy_core_secrets", max_retries=5,
             retry_delay=timedelta(minutes=2))
@track_step
def destroy_core_secrets(self, prior:dict, *, mr_iid: int | None = None):
    logger.debug("[delete_core_secrets] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
...
@shared_task(bind=True, name="workers.tasks.destroy.verify_core_secrets",
...
@shared_task(bind=True, name="workers.tasks.destroy.merge_core_secrets_mr",
             retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
