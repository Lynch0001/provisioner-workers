...
GIT_BRANCH_PREFIX = "auto/destroy-core-keycloak-"
SKIP_DESTROY_TASKS = True

@shared_task(bind=True, name="workers.tasks.destroy.delete_core_keycloak", queue="core")
@track_step
def delete_core_keycloak(self, prior:dict, **kwargs):
    logger.debug("[delete_core_keycloak] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[delete_core_keycloak] created branch %s", branch_name)

            repo_root = Path(gh.repo.working_tree_dir).resolve()

            rel_path = f"{INFR_BASE_PATH_DEMO}/{project_id}/keycloak"
            info = safe_delete_path(repo_root, rel_path)
            logger.info("[delete_core_keycloak] delete info: %s", info)
            ...
            if rc != 0:
                gh.commit_and_push(f"Delete core_keycloak for {project_id}")
            else:
                logger.info("[delete_core_keycloak] no staged changes; skipping commit/push")
            ...
            return {
                ...
                "branch_exists": False,
                "note": f"[delete:core_keycloak] started with {branch_name}"
            }
    except CeleryRetry:
        raise
...
@shared_task(bind=True, name="workers.tasks.destroy.create_core_keycloak_mr", queue="core")
@track_step
def create_core_keycloak_mr(self, prior:dict, **kwargs):
    ...
    try:
        ...
        mr = get_or_create_mr(
            ...
            squash=False,
            labels=["destroy", "core_keycloak"],
            description=f"Automated MR for `{project_id}`",
        )
        logger.info("keycloak MR ready: %s", getattr(mr, "web_url", "<no-url>"))
...
@shared_task(bind=True, name="workers.tasks.destroy.destroy_core_keycloak", max_retries=5,
             retry_delay=timedelta(minutes=2))
@track_step
def destroy_core_keycloak(self, prior:dict, *, mr_iid: int | None = None):
    logger.debug("[destroy_core_keycloak] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
    ...
    try:
        ...
        source_branch = f"{GIT_BRANCH_PREFIX}{project_id}"
        atlantis_dir = f"{INFR_BASE_PATH_DEMO}/{project_id}/keycloak"
...
@shared_task(bind=True, name="workers.tasks.destroy.verify_core_keycloak",
             retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
def verify_core_keycloak(self, prior: dict):
    if not isinstance(prior, dict) or "project_id" not in prior:
        prior = {"project_id": str(prior)}
    project_id = prior["project_id"]
    logger.warning(f"core_keycloak not verified")
    return {"project_id": project_id}


@shared_task(bind=True, name="workers.tasks.destroy.merge_core_keycloak_mr",
             retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
def merge_core_keycloak_mr(self, prior:dict, **kwargs):
    logger.debug("[merge_core_keycloak] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
    ...
    try:
        merge_results = merge_branch(
            ...
            log_prefix="[merge_core_keycloak_mr]",
        )
