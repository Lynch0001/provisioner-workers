...
GIT_BRANCH_PREFIX = "auto/destroy-core-registries-"

SKIP_VERIFY_TASKS = True
SKIP_DESTROY_TASKS = True

@shared_task(bind=True, name="workers.tasks.destroy.delete_core_registries", queue="core")
@track_step
def delete_core_registries(self, prior:dict, **kwargs):
    # Observe what Celery actually delivered
    logger.debug("[delete_core_registries] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[delete_core_registries] created branch %s", branch_name)

            repo_root = Path(gh.repo.working_tree_dir).resolve()

            rel_path = f"{INFR_BASE_PATH_DEMO}/rancher-registry-secrets/xib/{project_id}"
            info = safe_delete_path(repo_root, rel_path)
            logger.info("[delete_core_registries] delete info: %s", info)
            ...
            if rc != 0:
                gh.commit_and_push(f"Delete registries for {project_id}")
            else:
                logger.info("[delete_core_registries] no staged changes; skipping commit/push")
            return {
                "project_id": project_id,
                "workflow_id": prior["workflow_id"],
                "branch_name": branch_name,
                "branch_exists": False,
                "note": f"[delete:core_registries] started with {branch_name}"
            }
    except CeleryRetry:
        raise
...
@shared_task(bind=True, name="workers.tasks.destroy.create_core_registries_mr", queue="core")
@track_step
def create_core_registries_mr(self, prior:dict, **kwargs):
    ...
    try:
        ...
        mr = get_or_create_mr(
            ...
            squash=False,
            labels=["destroy", "core_registries"],
            description=f"Automated MR for `{project_id}`",
        )
        logger.info("Core_Registries MR ready: %s", getattr(mr, "web_url", "<no-url>"))
...
@shared_task(bind=True, name="workers.tasks.destroy.destroy_core_registries", max_retries=5,
             retry_delay=timedelta(minutes=2))
@track_step
def destroy_core_registries(self, prior:dict, *, mr_iid: int | None = None):
    if SKIP_DESTROY_TASKS:
        logger.warning("[destroy_core_registries] Skipping destroy registries")
        return {**prior}
    logger.debug("[destroy_core_registries] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
    ...
    try:
        ...
        source_branch = f"{GIT_BRANCH_PREFIX}{project_id}"
        atlantis_dir = f"{INFR_BASE_PATH_DEMO}/rancher-registry-secrets/xibus/{project_id}"
...
@shared_task(bind=True, name="workers.tasks.destroy.verify_core_registries",
             retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
def verify_core_registries(self, prior: dict):
    if SKIP_VERIFY_TASKS:
        logger.warning("[verify_core_registries] Skipping Verification of core_registries")
        return {**prior}
    ...
    secret_specs = [
        ...
        SecretSpec(namespace=namespace, names=["harbor-xibus"]),
    ]
...
@shared_task(bind=True, name="workers.tasks.destroy.merge_core_registries_mr",
             retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
def merge_core_registries_mr(self, prior:dict, **kwargs):
    logger.debug("[merge_core_registries] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
    ...
    try:
        merge_results = merge_branch(
            ...
            log_prefix="[merge_core_registries_mr]",
        )
