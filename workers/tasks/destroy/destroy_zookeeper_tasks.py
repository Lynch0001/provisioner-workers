...
from workers.handlers.git_handler import GitHandler, GitRepoSpec, safe_delete_path
...
from workers.handlers.verify_handler import WorkloadSpec
...
GIT_BRANCH_PREFIX = "auto/destroy-zk-"

SKIP_VERIFY_TASKS = True
SKIP_DESTROY_TASKS = True

@shared_task(bind=True, name="workers.tasks.destroy.delete_zookeeper_cluster", queue="zookeeper")
@track_step
def delete_zookeeper_cluster(self, prior:dict, **kwargs):
    # Observe what Celery actually delivered
    logger.debug("[delete_zookeeper] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    # normalize: tolerate kwargs/project_id dicts if needed
    project_id = prior["project_id"]
    if project_id is None:
        project_id = kwargs.get("project_id") or (kwargs.get("zk") or {}).get("project_id")
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[delete_zookeeper] created branch %s", branch_name)

            repo_root = Path(gh.repo.working_tree_dir).resolve()
            rel_path = f"{INFR_BASE_PATH_DEMO}/{project_id}/zookeeper"
            info = safe_delete_path(repo_root, rel_path)
            logger.info("[delete_zookeeper] delete info: %s", info)
            ...
            if rc != 0:
                gh.commit_and_push(f"Delete Zookeeper for {project_id}")
            else:
                logger.info("[delete_zookeeper] no staged changes; skipping commit/push")
            return {
                "project_id": project_id,
                "workflow_id": prior["workflow_id"],
                "branch_name": branch_name,
                "branch_exists": False,
                "note": f"[delete:zookeeper] started with {branch_name}"
            }
    except CeleryRetry:
        raise
...
@shared_task(bind=True, name="workers.tasks.destroy.create_zookeeper_mr", queue="zookeeper")
@track_step
def create_zookeeper_mr(self, prior:dict, **kwargs):
    ...
    try:
        ...
        mr = get_or_create_mr(
            ...
            squash=False,
            labels=["destroy", "zookeeper"],
            description=f"Automated MR for `{project_id}` (Zookeeper).",
        )
        logger.info("Zookeeper MR ready: %s", getattr(mr, "web_url", "<no-url>"))
...
@shared_task(bind=True, name="workers.tasks.destroy.destroy_zookeeper_cluster", queue="zookeeper", max_retries=5, retry_delay=timedelta(minutes=2))
@track_step
def destroy_zookeeper_cluster(self, prior:dict, *, mr_iid: int | None = None):
    if SKIP_DESTROY_TASKS:
        logger.warning("[destroy_zookeeper] Skipping destroy zookeeper")
        return {**prior}
    logger.debug("[destroy_zookeeper] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
    ...
    try:
        ...
        source_branch = f"{GIT_BRANCH_PREFIX}{project_id}"
        atlantis_dir = f"{INFR_BASE_PATH_DEMO}/{project_id}/zookeeper"
...
@shared_task(bind=True, name="workers.tasks.destroy.verify_zookeeper_cluster",
             queue="zookeeper", retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
def verify_zookeeper_cluster(self, prior: dict):
    if SKIP_VERIFY_TASKS:
        logger.warning("[verify_zookeeper] Skipping Verification of Zookeeper cluster")
        return {**prior}
    if not isinstance(prior, dict) or "project_id" not in prior:
        prior = {"project_id": str(prior)}
    project_id = prior["project_id"]
    namespace = f"{project_id}-zookeeper"
    workloads = [
        WorkloadSpec(kind="statefulset", namespace=namespace, label_selector="app=zookeeeper", min_ready_ratio=1.0),
    ...
    return run_verify_task(
        ...
        workloads=workloads,
        secrets=secrets,
        verify_key="verify_zookeeper",
        hard_fail_on_not_ready=False,
        retry_seconds=60,
    )


@shared_task(bind=True, name="workers.tasks.destroy.merge_zookeeper_mr",
             queue="zookeeper", retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
def merge_zookeeper_mr(self, prior:dict, **kwargs):
    project_id = prior.get("project_id")
