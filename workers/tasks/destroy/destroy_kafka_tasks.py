...
GIT_BRANCH_PREFIX = "auto/destroy-kfk-"

SKIP_VERIFY_TASKS = True
SKIP_DESTROY_TASKS = True

@shared_task(bind=True, name="workers.tasks.destroy.delete_kafka_cluster", queue="kafka")
@track_step
def delete_kafka_cluster(self, prior:dict, **kwargs):
    # Observe what Celery actually delivered
    logger.debug("[delete_kafka] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[delete_kafka] created branch %s", branch_name)

            repo_root = Path(gh.repo.working_tree_dir).resolve()

            rel_path = f"{INFR_BASE_PATH_DEMO}/{project_id}/kafka"
            info = safe_delete_path(repo_root, rel_path)
            logger.info("[delete_kafka] delete info: %s", info)
            ...
            if rc != 0:
                gh.commit_and_push(f"Destroy Kafka for {project_id}")
            else:
                logger.info("[delete_kafka] no staged changes; skipping commit/push")
            ...
            return {
                ...
                "branch_exists": False,
                "note": f"[delete:kafka] started with {branch_name}"
            }
    except CeleryRetry:
        raise
...
@shared_task(bind=True, name="workers.tasks.destroy.create_kafka_mr", queue="kafka")
@track_step
...
@shared_task(bind=True, name="workers.tasks.destroy.destroy_kafka_cluster", queue="kafka", max_retries=5,
             retry_delay=timedelta(minutes=2))
@track_step
def destroy_kafka_cluster(self, prior:dict, *, mr_iid: int | None = None):
    logger.debug("[destroy_kafka] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
...
@shared_task(bind=True, name="workers.tasks.destroy.verify_kafka_cluster",
...
def verify_kafka_cluster(self, prior: dict):
    if SKIP_VERIFY_TASKS:
        logger.warning("[verify_kafka] Skipping Verification of Kafka")
        return {**prior}
...
@shared_task(bind=True, name="workers.tasks.destroy.merge_kafka_mr",
             queue="kafka", retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
