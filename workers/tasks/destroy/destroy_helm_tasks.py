...
GIT_BRANCH_PREFIX = "auto/destroy-helm-"


@shared_task(bind=True, name="workers.tasks.destroy.delete_helm_values", queue="helm")
@track_step
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            if gh.remote_branch_exists(branch_name):
                ...
                return result
            gh.commit_and_push(f"Provisioning Helm Apps {project_id}")
            return {
                "project_id": project_id,
                "workflow_id": prior["workflow_id"],
                "branch_name": branch_name,
...
@shared_task(bind=True, name="workers.tasks.destroy.create_helm_values_mr", queue="helm")
@track_step
...
@shared_task(bind=True, name="workers.tasks.destroy.merge_helm_values_mr",
