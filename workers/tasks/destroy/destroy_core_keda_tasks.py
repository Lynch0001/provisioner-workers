...
GIT_BRANCH_PREFIX = "auto/destroy-core-keda-"

SKIP_VERIFY_TASKS = True

@shared_task(bind=True, name="workers.tasks.destroy.delete_core_keda", queue="core")
@track_step
def delete_core_keda(self, prior:dict, **kwargs):
    # Observe what Celery actually delivered
    logger.debug("[delete_core_keda] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    ...
    try:
        with GitHandler(spec, project_id) as gh:
            ...
            logger.info("[delete_core_keda] created branch %s", branch_name)

            repo_root = Path(gh.repo.working_tree_dir).resolve()

            rel_path = f"{INFR_BASE_PATH_DEMO}/{project_id}/keda-artemis-secret"
            info = safe_delete_path(repo_root, rel_path)
            logger.info("[delete_core_keda] delete info: %s", info)
            ...
            if rc != 0:
                gh.commit_and_push(f"Delete core_keda for {project_id}")
            else:
                logger.info("[delete_core_keda] no staged changes; skipping commit/push")
            ...
            return {
                ...
                "branch_exists": False,
                "note": f"[delete:core_keda] started with {branch_name}"
            }
    except CeleryRetry:
        raise
...
@shared_task(bind=True, name="workers.tasks.destroy.create_core_keda_mr", queue="core")
@track_step
def create_core_keda_mr(self, prior:dict, **kwargs):
    ...
    try:
        ...
        mr = get_or_create_mr(
            ...
            squash=False,
            labels=["destroy", "core_keda"],
            description=f"Automated MR for `{project_id}`",
        )
        logger.info("keda MR ready: %s", getattr(mr, "web_url", "<no-url>"))
...
@shared_task(bind=True, name="workers.tasks.destroy.destroy_core_keda", max_retries=5,
             retry_delay=timedelta(minutes=2))
@track_step
def destroy_core_keda(self, prior:dict, *, mr_iid: int | None = None):
    logger.debug("[destroy_core_keda] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
...
@shared_task(bind=True, name="workers.tasks.destroy.verify_core_keda",
...
@shared_task(bind=True, name="workers.tasks.destroy.merge_core_keda_mr",
             retry_backoff=3, retry_backoff_max=60,
             retry_jitter=True, retry_kwargs={"max_retries": 6})
@track_step
def merge_core_keda_mr(self, prior:dict, **kwargs):
    logger.debug("[merge_core_keda] args=%r kwargs=%r", self.request.args, self.request.kwargs)
    project_id = prior.get("project_id")
